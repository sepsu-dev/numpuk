        ...
        <section className="relative pt-28 pb-20 overflow-hidden bg-[#FEFEFE]">
            ...
            <div className="container max-w-5xl mx-auto px-8 relative z-10">
                ...
                <div className="max-w-2xl mx-auto text-center mb-16">
                    <div className="inline-flex items-center gap-2.5 px-4 py-2 rounded-md bg-black/[0.03] border border-black/[0.06] mb-10">
                        <span className="flex h-1.5 w-1.5 rounded-md bg-green-500 animate-pulse" />
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                        <Link
                            href="/register"
                            className="px-8 py-4 rounded-md bg-[#0A0A0A] text-white font-black text-[15px] shadow-[6px_6px_0_0_rgba(168,85,247,0.25)] hover:shadow-none hover:translate-x-1 hover:translate-y-1 transition-all active:scale-95 inline-flex items-center gap-3 group"
                        >
                            Mulai Gratis Sekarang
                            <span className="w-5 h-5 rounded-md bg-white/10 flex items-center justify-center group-hover:rotate-45 transition-transform text-[11px]">→</span>
                        </Link>
                        <Link
                            href="#features"
                            className="px-8 py-4 rounded-md border-2 border-black/10 text-[#0A0A0A] font-black text-[15px] hover:border-black transition-colors"
                        >
                            Lihat Fitur
                        </Link>
                    </div>
                ...
                <div className="relative animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-500">
                    ...
                    <div className="rounded-md border-2 border-black/[0.08] bg-white shadow-[0_32px_80px_rgba(0,0,0,0.08)] overflow-hidden">
                        ...
                        <div className="flex items-center gap-2 px-6 py-4 border-b border-black/[0.06] bg-[#FAFAFA]">
                            ...
                            <div className="mx-auto w-48 h-5 rounded-md bg-black/[0.04] flex items-center justify-center">
                            ...
                            <div className="w-48 shrink-0 bg-[#FAFAFA] border-r border-black/[0.06] p-5 hidden md:flex flex-col gap-6">
                                ...
                                <div className="space-y-1.5">
                                    {["Dashboard", "Proyek", "Kalender", "Tim"].map((item, i) => (
                                        <div key={i} className={`text-xs font-bold px-3 py-2 rounded-md ${i === 0 ? "bg-black/[0.05] text-[#0A0A0A]" : "text-muted-foreground/60"}`}>{item}</div>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="flex-1 p-7 overflow-hidden">
                                ...
                                <div className="grid grid-cols-3 gap-3 h-44 overflow-hidden">
                                    {[
                                        ...
                                        <div key={i} className={`${col.color} rounded-md p-3`}>
                                            <div className="text-[9px] font-black uppercase tracking-widest text-muted-foreground mb-2">{col.label}</div>
                                            <div className="space-y-1.5">
                                                {col.items.map((item, j) => (
                                                    <div key={j} className="bg-white rounded-md px-2.5 py-2 text-[10px] font-bold text-[#0A0A0A] shadow-sm border border-black/[0.04]">{item}</div>
                                                ))}
                                            </div>
                                        </div>
                                    ))}
